...
class QRScannerCheckInScreen extends StatefulWidget {
  ...
  @override
  _QRScannerCheckInScreenState createState() => _QRScannerCheckInScreenState();
}
        ...
        showDialog(
          ...
          builder: (context){
            return CustomConfirmationDialog(
              message: "Would you like to check in Flight No\n ${result!.code}",
              onYesConfirmTap: () async{
                ...
                crewMemberEntry = CrewScheduleEntry(
                    uid: currentUser!.uid,
                    flightNo: result!.code ?? '',
                    surname: userProfile!.fullName.split(' ').last,
                    staffNumber: userProfile.staffNumber,
                    checkInTime: Timestamp.now(),
                    isCheckedIn: true,
                    signatureIn: userProfile.fullName.split(' ').last,
                    checkOutTime: null,
                    signatureOut: null
                );

                FlightServices.checkIn(crewMemberEntry).then((entryResponse) {
                  ...
                  if (entryResponse == 'success') {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Check-in failed: $entryResponse'))
                    );
                  }
                }).catchError((error) {
                  Navigator.of(context).pop(); // Pop CustomLoader
